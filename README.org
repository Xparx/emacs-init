...
** My custom emacs-init
   ...
   It should be mostrly self contained, meaning that all files it fetches,
...
** Structure
   ...
   the elisp code embedded in this file.  =emacs-init.org= then
   loads some core configurations and packages which should be
   generally useful, and goes on to load user-specific
   configuration files from the following locations mentioned in
   [[customization]] immediately below.
