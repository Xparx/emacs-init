...
** My custom emacs-init
   ...
   creates and need should be in this directory.
...
**** El-getting org-mode
     ...
     :tangle:   yes
     :END:
     Install manually.  This file will not load untill a el-get
     ...
     #+begin_src emacs-lisp
       (setq org-capture-templates
             '(("t" "Todo" entry (file+headline "~/notebook/todo.org" "Tasks")
                "* TODO %?\n  %i\n  %a")
               ("n" "Journal" entry (file+headline "~/notebook/notes.org")
                "* %?\nEntered on %U\n  %i\n  %a")))
     #+end_src
...
*** Yasnippet
    ...
    Add loadpath to yasnippet from el-get. Seems pointless (it sure does. Should not be needed)
    #+begin_src emacs-lisp :tangle no
