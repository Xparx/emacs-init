...
Unnessessary with doing =(elpy-enable)=
#+begin_src emacs-lisp :tangle no
...
Not sure this is needed for most coding modes. It seems to be
built in to many of them /e.g./ python, julia etc
#+begin_src emacs-lisp
...
(when (and (el-get-package-exists-p "ess") (executable-find "julia"))
  (setq inferior-julia-program-name "julia") ;; need to be set before ob-julia is run
