...
(setq org-capture-templates
      '(("t" "Todo" entry (file+headline (car org-agenda-files) "Tasks")
        ...
        ("s" "Code Snippet" entry
         (file+headline (car org-default-notes-file) "Code")
         "* %?\t%^g\n#+begin_src %^{language}\n\n#+end_src")
        ("m" "Media" entry
         (file+headline (car org-default-notes-file) "Movies, TV shows and books")
         "* %?\nURL: \nEntered on %U\n")
...
Don't evaluate src code on export
#+begin_src emacs-lisp
...
**** Org LaTeX export with default packages                      :deprecated:
:PROPERTIES:
:header-args: :tangle   no
:END:
...
(when (el-get-package-exists-p "org-ref")
  ...
  (setq org-ref-bibliography-notes "~/research/bib-notes.org"
        ...
        org-ref-note-title-format "** TOREAD %k - %t
:PROPERTIES:
:Custom_ID: %k
:AUTHOR: %a
:JOURNAL: %j
